...
def extract_features(args):
    ...
    for sound_sample, audio_path in gen_audio_from_dir(args.input_dir,
        ...
        # embed, time_stamps = get_timestamp_embeddings(new_sample, model)
        # print(embed.shape)
        print(new_sample.shape)
        new_sample = new_sample.squeeze(3).squeeze(0)
        embed = get_scene_embeddings(new_sample, model)
        print(embed.shape)
