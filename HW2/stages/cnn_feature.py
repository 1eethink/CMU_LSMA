...
class CNNFeature(Stage):
    ...
    def extract_cnn_features(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        converted = frames.permute(0,3,1,2)
        with torch.no_grad():
            transformed = self.transforms(converted).to(self.device)
            # print(self.model(transformed)['feature'].shape)
            return self.model(transformed)['feature']
            # return None
