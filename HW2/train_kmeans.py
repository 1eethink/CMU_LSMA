...
def select_features(features: List[np.ndarray]) -> np.ndarray:
    ...
    features = [i for i in features if np.any(i)]
    features = np.concatenate(features, axis=0)
    index = np.random.randint(features.shape[0], size=128)
...
def main(args):
    ...
    kmeans = KMeans(args.num_clusters, random_state=args.seed, verbose=1, n_init = 3)
